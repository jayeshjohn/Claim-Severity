...
# Plot Loss
plot(x = 1:nrow(raw_data), y = raw_data$loss, type = "h", main = "Loss Plot", xlab = "Row Number", ylab = "Loss", col = viridis(20))
...
We notice that some of the Continuous variables like the Cat2, Cat3 appears to be variables that were originally Categorical. 
...
# Note that we intentionally used nrounds = 3 inorder to save some computational time, but for xgb.train we use the optimal nround value of 1101.
...
# Using the xgb.cv function to calculate the best nround for this model. 
# Note that we intentionally use nrounds = 3 inorder to save some computational time, but for xgb.train we use the optimal nround value of 1181 determined during prior execution.
...
# Conclusion 
 ...
 Accurate prediction of 'loss' holds key importance for Insurance companies, as it helps drive key business decisions and help reduce potential claim processing delays caused during the claims process. It can also help automate some of the manual review and claim adjudication steps reducing the overall claim processing lead time. This will ultimately hold a real value to end customers leading to better customer experience.
...
## Future Impovement
 ...
 Further improvements can be made to the models if we have the knowledge of the complete data features. For example Demographic information of the Insured, Driving History, Loss by State, date and time etc. will help apply domain knowledge for the modeling. Alternate models like Neural Networks can be looked at, but with the cost of high computational expense.
